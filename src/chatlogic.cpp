...
ChatLogic::ChatLogic()
{
    ...
    // TK done: move content fo LoadAnswerGraphFromFile function
...
ChatLogic::~ChatLogic()
{
    ...
    // TK done: remove delete in Task5 as _chatBot is now a unique_pointer
    //delete _chatBot;

    // TK done: delete no more necessary due to smart pointer
    // delete all nodes
    ...
    // TK done: remove _edges from chatlogic (destructor no more necessary as smart pointers)
    // delete all edges
...
void ChatLogic::LoadAnswerGraphFromFile(std::string filename)
{
            ...
            if (type != tokens.end())
            {
                ...
                if (idToken != tokens.end())
                {
                    ...
                    if (type->second == "NODE")
                    {
                        //// STUDENT CODE
                        ////
                        // TK done: modify line "auto newNode" by repyling a refernce to unique ptr
                    ...
                    if (type->second == "EDGE")
                    {
                        ...
                        if (parentToken != tokens.end() && childToken != tokens.end())
                        {
                            ...
                            // TK done: modify lines "auto parent"/"auto child" by repyling a refernce to unique ptr
                            ...
                            // TK done: remove _edges from chatlogic.cpp
    ...
    std::cout<<"chatlogic.cpp line 229"<<std::endl;
    chatBot.SetRootNode(rootNode);
    std::cout<<"chatlogic.cpp line 231"<<std::endl;
    rootNode->MoveChatbotHere(std::move(chatBot));
    std::cout<<"chatlogic.cpp line 233"<<std::endl;
...
// TK done: remove this function: _chatBot does not exist anymore in ChatLogic
